...
- (void)updateBar
{
    ...
    for (NSNumber *valueString in _yValues) {
        ...
        if (_bars.count == _yValues.count) {
            ...
            if (_barWidth) {
                barWidth = _barWidth;
                //手动设置轴的宽 中心X在label中心X
                barXPosition = index *  _xLabelWidth + _chartMarginLeft + _xLabelWidth /2.0 - _barWidth /2.0;
            }else{
                //barXPosition = index *  _xLabelWidth + _chartMarginLeft + _xLabelWidth * 0.2;
                
                //设置轴的宽
                if (_showLabel) {
                    barWidth = _xLabelWidth * 0.4; //0.5;
                }
                else {
                    barWidth = _xLabelWidth * 0.4; //0.6;
                }
                
                if (index%2==0) {
                    barXPosition = _chartMarginLeft+_xLabelWidth*0.2 +index/2*_xLabelWidth;
                    
                }else{
                    barXPosition = (index-1)/2 *  _xLabelWidth + _chartMarginLeft+_xLabelWidth*0.6;
                }
                //NSLog(@"--%f",barXPosition);
            }
