...
- (void)setYLabels {
    ...
    if (yStep == 0.0) {
        PNChartLabel *minLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0,_chartCavanHeight,  _chartMarginLeft, _yLabelHeight)];
        minLabel.text = [self formatYLabel:0.0];
        [self setCustomStyleForYLabel:minLabel];
        [self addSubview:minLabel];
        [_yChartLabels addObject:minLabel];

        PNChartLabel *midLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, (_chartCavanHeight / 2), _chartMarginLeft, _yLabelHeight)];
        midLabel.text = [self formatYLabel:_yValueMax];
        [self setCustomStyleForYLabel:midLabel];
        [self addSubview:midLabel];
        [_yChartLabels addObject:midLabel];

        PNChartLabel *maxLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, 0.0,_chartMarginLeft,  _yLabelHeight)];
        ...
        while (num > 0) {
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0,  _chartCavanHeight - index * yStepHeight, _chartMarginLeft, _yLabelHeight)];
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            CGFloat y =  (_chartCavanHeight - index * yStepHeight);

            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, y,_chartMarginLeft ,_yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            CGFloat x = (index * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0+10);
            //CGFloat y =_chartCavanHeight+10;
            CGFloat y =_chartMarginBottom + _chartCavanHeight;
            
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(x, y, _xLabelWidth,_chartMarginBottom)];
...
- (void)strokeChart {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        chartLine.path = progressline.CGPath; //绘制线条
        pointLayer.path = pointPath.CGPath;   //绘制折点处图形
...
//        if (self.showChartShadow) {  //阴影
//            CAShapeLayer *apointLayer = [CAShapeLayer layer];
//            CAShapeLayer *apointLayer1 = [CAShapeLayer layer];
//            UIBezierPath *aPath = [UIBezierPath bezierPath];
//            for (NSInteger j=0; j<self.pathPoints.count; j++) {
//    
//                NSArray *arr = self.pathPoints[j];
//    
//                for (NSInteger i=0; i<arr.count; i++) {
//                    CGPoint point = [arr[i] CGPointValue];
//                    if (i==0) {  //第一个点
//                        [aPath moveToPoint:CGPointMake(point.x, _chartCavanHeight+_yLabelHeight/2)];
//                    }
//                    [aPath addLineToPoint:CGPointMake(point.x ,point.y)];
//                    if (i==arr.count-1){  //最后一个点
//                        [aPath addLineToPoint:CGPointMake(point.x, _chartCavanHeight+_yLabelHeight/2)];
//                    }
//                }
//                //[@[[UIColor orangeColor],[UIColor purpleColor]][j] setFill];
//                //[aPath fill];
//                //[[UIColor clearColor] setStroke];
//                [aPath closePath];
//                [@[[UIColor orangeColor],[UIColor purpleColor]][j] setFill];
//                if (j==0) {
//                    apointLayer.strokeColor = [[UIColor orangeColor] CGColor];
//                    apointLayer.path=[aPath CGPath];
//                    apointLayer.fillColor = [[UIColor redColor]CGColor];
//                    apointLayer.lineWidth = 1;
//                    [self.layer addSublayer:apointLayer];
//                }else{
//                    apointLayer1.strokeColor = [[UIColor purpleColor] CGColor];
//                    apointLayer1.path=[aPath CGPath];
//                    apointLayer1.fillColor = [[UIColor orangeColor]CGColor];
//                    apointLayer1.lineWidth = 1;
//                    [self.layer addSublayer:apointLayer1];
//    
//    
//                }
//            }
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath {
    ...
    // Draw each line  chartData数组装的PNLineChartData对象
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        PNLineChartData *chartData = self.chartData[lineIndex];
        ...
        //chartPath  pointsPath 俩个初始化后的数组
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            int x = i * _xLabelWidth + _chartMarginLeft + 10;  //调整X轴起点
...
- (void)setChartData:(NSArray *)data {
    if (data != _chartData) {
        ...
        for (PNLineChartData *chartData in data) {
            ...
            chartLine.fillColor=[[UIColor purpleColor]CGColor];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.isShowCoordinateAxis) {
        ...
        //x轴长度  y轴高度
        CGFloat xAxisWidth = CGRectGetWidth(rect) - (_chartMarginLeft + _chartMarginRight)/2 +5 ;
        CGFloat yAxisHeight = _chartMarginBottom + _chartCavanHeight;

        // draw coordinate axis 坐标轴
        ...
        CGContextMoveToPoint(ctx, _chartMarginLeft+10, 0);
        CGContextAddLineToPoint(ctx, _chartMarginLeft+10, _chartCavanHeight+_chartMarginBottom);
        CGContextAddLineToPoint(ctx, CGRectGetWidth(rect)-_chartMarginRight, _chartCavanHeight+_chartMarginBottom);
...
//        if (self.showLabel) {
//            // draw x axis separator 分割的小短线
//            CGPoint point;
//            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
//                point = CGPointMake(2 * _chartMarginLeft + (i * _xLabelWidth), _chartMarginBottom + _chartCavanHeight);
//                //point.x+3 调整x轴分割线
//                CGContextMoveToPoint(ctx, point.x+3, point.y - 2);
//                CGContextAddLineToPoint(ctx, point.x+3, point.y);
//                CGContextStrokePath(ctx);
//            }
//
...
//*********************************************
    ...
    if (self.showBreakpoint) {
        //分割的小短线  X轴
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        CGPoint point;
        for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
            point = CGPointMake( _chartMarginLeft +_xLabelWidth+10 + i*_xLabelWidth, _chartCavanHeight+_yLabelHeight/2);
    ...
    if (self.showYGridLines) {
        ...
        //水平方向线条  Y轴label宽度就是_chartMarginLeft 离第一根线条间距10
        for (NSUInteger i = 0; i < _yLabelNum+1; i++) {
            ...
            CGContextMoveToPoint(ctx, point.x, point.y);
            CGContextAddLineToPoint(ctx, _chartMarginLeft+10+self.xLabels.count* _xLabelWidth, point.y);
            CGContextStrokePath(ctx);
        }
        
        //左边第一条线条
        CGPoint topLine = CGPointMake(_chartMarginLeft + yAxisOffset, (_chartCavanHeight + _yLabelHeight / 2));
        CGPoint bottomLine = CGPointMake(_chartMarginLeft + yAxisOffset, (_chartCavanHeight -_yLabelNum* yStepHeight  + _yLabelHeight / 2));
        CGContextMoveToPoint(ctx, topLine.x, topLine.y);
        CGContextAddLineToPoint(ctx, bottomLine.x, bottomLine.y);
        CGContextStrokePath(ctx);
        ...
        for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
            point = CGPointMake( _chartMarginLeft+_xLabelWidth+10+i * _xLabelWidth, _chartCavanHeight+_yLabelHeight/2);
            CGContextMoveToPoint(ctx, point.x, point.y);
            CGContextAddLineToPoint(ctx, point.x, _yLabelHeight / 2);
            CGContextStrokePath(ctx);
        }
    }
    ...
    //画板上画  没有动画效果 
    /*if (self.showChartShadow) {  //阴影
        if (_shadowColors.count<=0) {
            self.shadowColors=@[[UIColor orangeColor],[UIColor purpleColor]];
        }
        ...
        for (NSInteger j=0; j<self.pathPoints.count; j++) {
            ...
            for (NSInteger i=0; i<arr.count; i++) {
                CGPoint point = [arr[i] CGPointValue];
                if (i==0) {  //第一个点
                    CGContextMoveToPoint(context,point.x, _chartCavanHeight+_yLabelHeight/2);
                }
                
                CGContextAddLineToPoint(context,point.x, point.y);
                
                if (i==arr.count-1){  //最后一个点
                    CGContextAddLineToPoint(context,point.x, _chartCavanHeight+_yLabelHeight/2);
                }
            }
            CGContextClosePath(context);//路径结束标志，不写默认封闭
//            if (_shadowColors.count<self.pathPoints.count) {
//                [[UIColor orangeColor] setFill];
//            }else{
//                [_shadowColors[j] setFill]; //设置填充色
//            }
...
- (void)setupDefaultValues {
    ...
    _chartMarginLeft =25.0;   //左边距离视图距离  和y轴label长度
    _chartMarginRight =25.0;
    _chartMarginTop =25.0;  //距离视图下边线高度
    _chartMarginBottom =25.0;   //划线距离x轴高度
